...
const LogReaction: React.FC = () => {
  ...
  const analyzeWithAI = async () => {
    ...
    try {
      ...
      const analysis = await GeminiService.analyzeClinicalSymptoms(symptoms, symptomDesc, timeSinceCondition);
  ...
  const fetchBarcodeIngredients = async (idx: number) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`OpenFoodFacts HTTP ${response.status}${text ? `: ${text}` : ''}`);
      }
      ...
      // Prefer structured ingredients array; fall back to ingredients_text fields when missing.
      const ingredientsFromArray =
        Array.isArray(product.ingredients) && product.ingredients.length > 0
          ? product.ingredients.map((ing: any) => ing?.text).filter(Boolean)
          : [];

      const ingredientsTextFallback =
      ...
      // Also get allergens
      const allergensArr = Array.isArray(product.allergens_tags)
      ...
      const message = err instanceof Error ? err.message : 'Error fetching barcode.';
      console.error('Barcode lookup error:', err);
      setProducts(p => p.map((pr, i) => i === idx ? { ...pr, isBarcodeLoading: false, barcodeError: message } : pr));
    }
  };
  ...
  const fetchSafeFoodBarcodeIngredients = async (idx: number) => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Error fetching barcode.';
      console.error('Safe food barcode lookup error:', err);
      setSafeFoodProducts(p => p.map((pr, i) => i === idx ? { ...pr, isBarcodeLoading: false, barcodeError: message } : pr));
    }
  };
                                  ...
                                  <input
                                    ...
                                    onChange={async e => {
                                      ...
                                      try {
                                        ...
                                        if (!API_NINJAS_KEY) throw new Error('Missing REACT_APP_API_NINJAS_KEY');
                              ...
                              {(() => {
                                if (product.scanText && !product.isListing && !product.listError && !product.isEditingOrDeleting && !product.hasProcessedScanText) {
                                  ...
                                  (async () => {
                                    ...
                                    try {
                                      const geminiResponse = await GeminiService.extractIngredients(product.scanText);
                                      const list = cleanAIResponse(geminiResponse);
                                        if (list.length === 0) {
